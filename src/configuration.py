...
class DataConfig:
    ...
    target_filename: str = "era5_1deg_merged.nc" #"era5_ensemble_3deg_mon.nc" #'era5_3deg.nc'
    ...
    # Per-variable data  preprocessing transforms
    transforms: List = field(default_factory= lambda: ['normalize_minus1_to_plus1'])

    transforms_per_variable: dict = field(default_factory= lambda: {
            'precipitation': ['log', 'normalize_minus1_to_plus1'],
            'temperature': ['normalize_minus1_to_plus1'],
            'total_cloud_cover': ['normalize_minus1_to_plus1']
            })

    epsilon_per_variable: dict = field(default_factory= lambda: {
            'precipitation': 0.0001,
            'temperature': 0.0001, 
            'total_cloud_cover': 0.0001
            })

    predict_variable: List[str] = field(default_factory= lambda: ['precipitation', 'temperature', 'total_cloud_cover'])
    ...
    pad_input: int = field(default=(4, 4, 6, 6))#field(default=(0, 0, 2, 2))

    lazy: bool = False
    
    #Print screen interval 
...
class DiffusionConfig:
    ...
    spherical_sampling: str = 'equiangular'     # 'equiangular' or 'healpix'
    spherical_depth: int = 4                    # number of encoder/decoder levels
    laplacian_type: str = 'combinatorial'       # 'combinatorial' or 'normalized'
    chebyshev_K: int = 3                        # Chebyshev polynomial order
    spherical_channels: tuple = field(default=(128, 128, 256, 256))  # feature widths per level (matches 2D UNet)
    time_emb_dim: int = 64                      # sinusoidal time embedding dimension
    use_coord_channels: bool = False             # append sin/cos(lat/lon) coordinate channels to break longitude symmetry
...
def parse_command_line():
    ...
    parser.add_argument("-ui", "--update_interval",
                        help="Interval of printing training information", type=int)

    parser.add_argument("-bt", "--backbone_type",
                        help="Backbone network type: 'unet2d' or 'spherical'", type=str)

    parser.add_argument("-ss", "--spherical_sampling",
                        help="Spherical sampling: 'equiangular' or 'healpix'", type=str)

    parser.add_argument("-sd", "--spherical_depth",
                        help="Number of encoder/decoder levels in spherical U-Net", type=int)

    parser.add_argument("-lt", "--laplacian_type",
                        help="Laplacian type: 'combinatorial' or 'normalized'", type=str)

    parser.add_argument("-ck", "--chebyshev_K",
                        help="Chebyshev polynomial order for graph convolutions", type=int)

    parser.add_argument("-sc", "--spherical_channels", nargs='+',
                        help="List of channel widths per spherical U-Net level", type=int)

    parser.add_argument("-ted", "--time_emb_dim",
                        help="Sinusoidal time embedding dimension", type=int)

    parser.add_argument("--use_coord_channels",
                        help="Append sin/cos(lat/lon) coordinate channels to spherical U-Net input",
                        action='store_true')
    ...
    print (f"Configuration: {config.update_interval}")
