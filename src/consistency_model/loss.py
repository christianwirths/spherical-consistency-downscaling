...
class PerceptualLoss(nn.Module):
    ...
    def forward(self, input: torch.tensor, target: torch.tensor):
        ...
        # Handle both 1-channel and 3-channel cases
        if upscaled_input.shape[1] == 1:
        ...
        elif upscaled_input.shape[1] == 3:
            # Already 3 channels - pass as-is to LPIPS
            # Note: LPIPS is trained on RGB, but should work reasonably with any 3-channel input
            pass
        else:
            raise ValueError(f"Unexpected number of channels: {upscaled_input.shape[1]}")
