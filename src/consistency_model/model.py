...
class Consistency(LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        model = self._build_backbone(config)
        ...
        if loss_func == "LPIPS":
            ...
            raise ValueError(f"Loss function '{loss_func}' not defined. Use 'LPIPS' or 'MSE'.")
    ...
    @staticmethod
    def _build_backbone(config: Config) -> nn.Module:
        """Construct the denoising backbone based on ``config.backbone_type``.

        Returns either a diffusers ``UNet2DModel`` (default) or a
        ``SphericalUNetWrapper`` (graph-based spherical U-Net).
        """
        if getattr(config, "backbone_type", "unet2d") == "spherical":
            # Resolve padded image dimensions from sample_dimension + pad_input
            h, w = config.sample_dimension
            return SphericalUNetWrapper(
                in_channels=config.in_channels,
                out_channels=config.out_channels,
                image_height=h,
                image_width=w,
                channel_list=config.spherical_channels,
                spherical_sampling=config.spherical_sampling,
                spherical_depth=config.spherical_depth,
                laplacian_type=config.laplacian_type,
                chebyshev_K=config.chebyshev_K,
                time_emb_dim=config.time_emb_dim,
                use_coord_channels=getattr(config, 'use_coord_channels', False),
            )
        else:
            return UNet2DModel(
                in_channels=config.in_channels,
